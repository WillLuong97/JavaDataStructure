...
//Approach:
Bottom-up Dynamic Programming becasue we need to calculate the sub problem of finding
the minimum time to get out of the current station, and by the time we exist each
assembly line, our algorithm should be able to pick up the minimum time
...
public class AssemblyLineScheduling {
    public int carAssembly(int a[][],  int t[][], int e[], int x[], int stations){
        //create two dp array to cache the current minimum time for each station in each line to process the cars
        ...
        for(int i = 1; i < stations; ++i){
            //finding the minimum time for each station on line 1 to process the cars
